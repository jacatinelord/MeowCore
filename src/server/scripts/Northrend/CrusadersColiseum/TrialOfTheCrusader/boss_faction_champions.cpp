...
struct boss_faction_championsAI : public ScriptedAI
{
    ...
    void JustEngagedWith(Unit * /*who*/) override
    {
        ...
        events.ScheduleEvent(EVENT_RESET_THREAT_AND_SWITCH_TARGET, 8000); // Schedule the event for the first time after 8 seconds
    }
    ...
    void ResetThreatAndSwitchTarget()
    {
        if (me->IsInCombat())
        {
            DoResetThreatList();
            Unit *target = SelectTarget(SelectTargetMethod::Random, 0, 90.0f, true);
            //me->Attack(target, true);
            me->SetInCombatWith(target);

            //me->Say("Selecting new Target", LANG_UNIVERSAL);
        }
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        // std::cout << "[TOC Diff] diff:        " << diff << std::endl;
        // std::cout << "[TOC ThreatTimer] threatTimer:        " << threatTimer << std::endl;
        if (threatTimer > 7000)
        {
            RecalculateThreat();
            ResetThreatAndSwitchTarget();
        }
        if (threatTimer <= diff)
        {
            // me->Say("This is the Recalculate Threat Event with time diff", LANG_UNIVERSAL);
            RecalculateThreat();
            threatTimer = urand(8750, 9250);
        }
        else
            // me->Say("This is the Recalculate Threat Event on else", LANG_UNIVERSAL);
            threatTimer -= diff;
