...
private:
    ...
    static void GetBotClassNameAndColor(uint8 botclass, std::string &bot_color_str, std::string &bot_class_str)
    {
        ...
        case BOT_CLASS_WARRIOR:
            bot_color_str = "ffc79c6e";
            bot_class_str = "Warrior";
            break;
        case BOT_CLASS_PALADIN:
            bot_color_str = "fff58cba";
            bot_class_str = "Paladin";
            break;
        case BOT_CLASS_HUNTER:
            bot_color_str = "ffabd473";
            bot_class_str = "Hunter";
            break;
        case BOT_CLASS_ROGUE:
            bot_color_str = "fffff569";
            bot_class_str = "Rogue";
            break;
        case BOT_CLASS_PRIEST:
            bot_color_str = "ffffffff";
            bot_class_str = "Priest";
            break;
        case BOT_CLASS_DEATH_KNIGHT:
            bot_color_str = "ffc41f3b";
            bot_class_str = "Death Knight";
            break;
        case BOT_CLASS_SHAMAN:
            bot_color_str = "ff0070de";
            bot_class_str = "Shaman";
            break;
        case BOT_CLASS_MAGE:
            bot_color_str = "ff69ccf0";
            bot_class_str = "Mage";
            break;
        case BOT_CLASS_WARLOCK:
            bot_color_str = "ff9482c9";
            bot_class_str = "Warlock";
            break;
        case BOT_CLASS_DRUID:
            bot_color_str = "ffff7d0a";
            bot_class_str = "Druid";
            break;
        case BOT_CLASS_BM:
            bot_color_str = "ffa10015";
            bot_class_str = "Blademaster";
            break;
        case BOT_CLASS_SPHYNX:
            bot_color_str = "ff29004a";
            bot_class_str = "Obsidian Destroyer";
            break;
        case BOT_CLASS_ARCHMAGE:
            bot_color_str = "ff028a99";
            bot_class_str = "Archmage";
            break;
        case BOT_CLASS_DREADLORD:
            bot_color_str = "ff534161";
            bot_class_str = "Dreadlord";
            break;
        case BOT_CLASS_SPELLBREAKER:
            bot_color_str = "ffcf3c1f";
            bot_class_str = "Spellbreaker";
            break;
        case BOT_CLASS_DARK_RANGER:
            bot_color_str = "ff3e255e";
            bot_class_str = "Dark Ranger";
            break;
        case BOT_CLASS_NECROMANCER:
            bot_color_str = "ff9900cc";
            bot_class_str = "Necromancer";
            break;
        case BOT_CLASS_SEA_WITCH:
            bot_color_str = "ff40d7a9";
            bot_class_str = "Sea Witch";
            break;
        case BOT_CLASS_CRYPT_LORD:
            bot_color_str = "ff19782b";
            bot_class_str = "Crypt Lord";
            break;
        default:
            bot_color_str = "ffffffff";
            bot_class_str = "Unknown";
            break;
        }
    }
...
#define MV_PRED9(skinm, skinf, facem, facef, hairm, hairf, hairc, featm, featf) \
    if constexpr (std::is_same_v<E, PlayerVisuals::Skins>)                      \
        return !F ? skinm : skinf;                                              \
    else if constexpr (std::is_same_v<E, PlayerVisuals::Faces>)                 \
        return !F ? facem : facef;                                              \
    else if constexpr (std::is_same_v<E, PlayerVisuals::HairStyles>)            \
        return !F ? hairm : hairf;                                              \
    else if constexpr (std::is_same_v<E, PlayerVisuals::HairColors>)            \
        return !F ? hairc : hairc;                                              \
    else if constexpr (std::is_same_v<E, PlayerVisuals::Features>)              \
    return !F ? featm : featf

        constexpr bool F = G == GENDER_FEMALE;
        if constexpr (R == RACE_HUMAN)
        {
            MV_PRED9(9, 9, 11, 14, 16, 23, 9, 8, 6);
        }
        if constexpr (R == RACE_DWARF)
        {
            MV_PRED9(8, 8, 9, 9, 15, 18, 9, 10, 5);
        }
        if constexpr (R == RACE_NIGHTELF)
        {
            MV_PRED9(8, 8, 8, 8, 11, 11, 7, 5, 9);
        }
        if constexpr (R == RACE_GNOME)
        {
            MV_PRED9(4, 4, 6, 6, 11, 11, 8, 7, 6);
        }
        if constexpr (R == RACE_DRAENEI)
        {
            MV_PRED9(13, 13, 9, 9, 13, 15, 6, 7, 6);
        }
        if constexpr (R == RACE_ORC)
        {
            MV_PRED9(8, 8, 8, 8, 11, 12, 7, 10, 6);
        }
        if constexpr (R == RACE_UNDEAD_PLAYER)
        {
            MV_PRED9(5, 5, 9, 9, 14, 14, 9, 16, 7);
        }
        if constexpr (R == RACE_TAUREN)
        {
            MV_PRED9(18, 10, 4, 3, 12, 11, 2, 6, 4);
        }
        if constexpr (R == RACE_TROLL)
        {
            MV_PRED9(5, 5, 4, 5, 9, 9, 9, 10, 5);
        }
        if constexpr (R == RACE_BLOODELF)
        {
            MV_PRED9(9, 9, 9, 9, 15, 18, 9, 9, 10);
        }
...
#define VISUALS_PRED1(r) (gender == GENDER_FEMALE) ? (                                                                           \
        ...
        {
        case RACE_HUMAN:
            return VISUALS_PRED1(RACE_HUMAN);
        case RACE_DWARF:
            return VISUALS_PRED1(RACE_DWARF);
        case RACE_NIGHTELF:
            return VISUALS_PRED1(RACE_NIGHTELF);
        case RACE_GNOME:
            return VISUALS_PRED1(RACE_GNOME);
        case RACE_DRAENEI:
            return VISUALS_PRED1(RACE_DRAENEI);
        case RACE_ORC:
            return VISUALS_PRED1(RACE_ORC);
        case RACE_UNDEAD_PLAYER:
            return VISUALS_PRED1(RACE_UNDEAD_PLAYER);
        case RACE_TAUREN:
            return VISUALS_PRED1(RACE_TAUREN);
        case RACE_TROLL:
            return VISUALS_PRED1(RACE_TROLL);
        case RACE_BLOODELF:
            return VISUALS_PRED1(RACE_BLOODELF);
        default:
            return false;
        }
#undef VISUALS_PRED1
...
#define FILL_VISUALS_REPORT2(s, r) s                                                                                             \
        ...
        for (uint8 race : {RACE_HUMAN, RACE_DWARF, RACE_NIGHTELF, RACE_GNOME, RACE_DRAENEI, RACE_ORC, RACE_UNDEAD_PLAYER, RACE_TAUREN, RACE_TROLL, RACE_BLOODELF})
        {
            ...
            {
            case RACE_HUMAN:
                FILL_VISUALS_REPORT2(stream, RACE_HUMAN);
                break;
            case RACE_DWARF:
                FILL_VISUALS_REPORT2(stream, RACE_DWARF);
                break;
            case RACE_NIGHTELF:
                FILL_VISUALS_REPORT2(stream, RACE_NIGHTELF);
                break;
            case RACE_GNOME:
                FILL_VISUALS_REPORT2(stream, RACE_GNOME);
                break;
            case RACE_DRAENEI:
                FILL_VISUALS_REPORT2(stream, RACE_DRAENEI);
                break;
            case RACE_ORC:
                FILL_VISUALS_REPORT2(stream, RACE_ORC);
                break;
            case RACE_UNDEAD_PLAYER:
                FILL_VISUALS_REPORT2(stream, RACE_UNDEAD_PLAYER);
                break;
            case RACE_TAUREN:
                FILL_VISUALS_REPORT2(stream, RACE_TAUREN);
                break;
            case RACE_TROLL:
                FILL_VISUALS_REPORT2(stream, RACE_TROLL);
                break;
            case RACE_BLOODELF:
                FILL_VISUALS_REPORT2(stream, RACE_BLOODELF);
                break;
            default:
                break;
            }
...
public:
        ...
        static ChatCommandTable npcbotDeleteCommandTable =
            {
                ...
                {"new", HandleNpcBotDeleteByNewCommand, rbac::RBAC_PERM_COMMAND_NPCBOT_DELETE, Console::Yes},
    ...
    static bool HandleNpcBotWPGenerateCommand(ChatHandler *handler, Optional<bool> save)
    {
        ...
        for (AreaPOIEntry const *aProto : sAreaPOIStore)
        {
            ...
            poiName.erase(std::remove_if(std::begin(poiName), std::end(poiName), [](char c)
                                         { return c == '\''; }),
                          poiName.end());
    ...
    static bool HandleNpcBotWPMoveCommand(ChatHandler *handler, Optional<uint32> wpId)
    {
        ...
        WanderNode *wp = (wpc && wpc->GetTypeId() == TYPEID_UNIT) ? WanderNode::FindInAllWPs(wpc->ToCreature()) : wpId ? WanderNode::FindInAllWPs(*wpId)
                                                                                                                       : nullptr;
        if (!wp)
        {
    ...
    static bool HandleNpcBotOrderCastCommand(ChatHandler *handler, Optional<std::string> bot_name, Optional<std::string> spell_name, Optional<std::string_view> target_token)
    {
        ...
        else
        {
            ...
            bot = ccBots.empty() ? nullptr : ccBots.size() == 1 ? ccBots.front()
                                                                : Acore::Containers::SelectRandomContainerElement(ccBots);
            if (!bot)
                bot = cBots.empty() ? nullptr : cBots.size() == 1 ? cBots.front()
                                                                  : Acore::Containers::SelectRandomContainerElement(cBots);
    ...
    static bool HandleNpcBotRecallSpawnsCommand(ChatHandler *handler)
    {
        ...
        if (owner->HaveBot())
            botvec.erase(std::remove_if(botvec.begin(), botvec.end(), [=](ObjectGuid botguid)
                                        { return owner->GetBotMgr()->GetBot(botguid); }),
                         botvec.end());
    ...
    static bool HandleNpcBotLookupCommand(ChatHandler *handler, Optional<uint8> botclass, Optional<bool> unspawned, Optional<uint8> teamid)
    {
        ...
        std::sort(botlist.begin(), botlist.end(), [](BotInfo const &bi1, BotInfo const &bi2)
                  { return bi1.id < bi2.id; });
    ...
    static bool HandleNpcBotDeleteCommand(ChatHandler *handler)
    {
        ...
        ObjectGuid receiver =
            botowner ? botowner->GetGUID() : bot->GetBotAI()->GetBotOwnerGuid() != 0 ? ObjectGuid(HighGuid::Player, 0, bot->GetBotAI()->GetBotOwnerGuid())
                                                                                     : chr->GetGUID();
    ...
    static bool HandleNpcBotDeleteByNewCommand(ChatHandler *handler, Optional<uint32> creature_id)
    {
        if (!creature_id)
        {
            handler->SendSysMessage(".npcbot delete new");
            handler->SendSysMessage("Deletes npcbot spawn from world and DB using creature id");
            handler->SetSentErrorMessage(true);
            return false;
        }
    ...
    static bool HandleNpcBotInfoCommand(ChatHandler *handler, Optional<Variant<uint32, std::string>> player_lg_name)
    {
        ...
        ObjectGuid master_guid = cached_guid ? cached_guid : (player_lg_name && player_lg_name->holds_alternative<uint32>()) ? ObjectGuid::Create<HighGuid::Player>(player_lg_name->get<uint32>())
                                                         : player && player->GetTarget().IsPlayer()                          ? player->GetTarget()
                                                                                                                             : ObjectGuid::Empty;
        ...
        if (map)
        {
            guidvec.erase(std::remove_if(std::begin(guidvec), std::end(guidvec),
                                         [bmap = map](ObjectGuid guid)
                                         { return bmap->find(guid) != bmap->end(); }),
                          std::end(guidvec));
        }
